...
class App extends Component {
  ...
  render() {
    const isSimpson = this.state.simpsonsOnly
      ? this.state.quoteList.filter(
          character => character.character.split(' ')[1] === 'Simpson'
        )
      : this.state.quoteList;
    return (
      <div>
        {isSimpson.map((quote, index) => (
          <QuoteCard {...quote} />
        ))}
        <button onClick={this.wantSimpson}>Simpsons only</button>
      </div>
    );
  }
}
